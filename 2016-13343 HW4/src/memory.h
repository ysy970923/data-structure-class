...
private:
	...
	int Alloc();
	void wipeMemory();
	void FreeMemory(int root);
	void Print();
};

/*
* used when error occured and memory needs to be wiped
*/
void Memory::wipeMemory() {
	free_list_root = 1;
	for (int i = 0; i < MEMORY_SIZE - 1; i++) {
		memoryList[i].lchild = 0;
		memoryList[i].rchild = i + 2;
	}
	memoryList[MEMORY_SIZE - 1].rchild = 0;
}
...
/*
* frees the memory tree used from root back to free list
* free the memory from the leaves of the tree
*/
void Memory::FreeMemory(int root) {
	int rootIndex = root - 1;
	if (root > 0) {
		FreeMemory(memoryList[rootIndex].lchild);
		if (memoryList[rootIndex].rchild != 0)
			FreeMemory(memoryList[rootIndex].rchild);
		int temp = memoryList[free_list_root - 1].rchild;
		memoryList[free_list_root - 1].rchild = root;
		memoryList[rootIndex].rchild = temp;
	}
}
...
void Memory::Print() {
	...
	for (int i = 0; i < MEMORY_SIZE; i++) {
		cout << "Node ID " << i + 1 << ": " << memoryList[i].lchild << ", " << memoryList[i].rchild << endl;
	}
}
