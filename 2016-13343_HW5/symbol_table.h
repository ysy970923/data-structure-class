...
private:
	...
	void changePointer(int hashVal, int toWhat, bool cacheTrue);
	...
	SymbolTable(HistoryStack & history) {
...
void SymbolTable::changePointer(int hashVal, int toWhat) {
	if (isNumber(hashVal))
		throw NUMBER_POINTER_ERROR;
	history->Push(hashVal, hashTable[hashValToIndex(hashVal)].pointer);
	hashTable[hashValToIndex(hashVal)].pointer = toWhat;
}

// change pointer to a given value
void SymbolTable::changePointer(int hashVal, int toWhat, bool cacheTrue) {
	if (isNumber(hashVal))
		throw NUMBER_POINTER_ERROR;
	hashTable[hashValToIndex(hashVal)].pointer = toWhat;
}
